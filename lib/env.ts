...
const getEnv = () => {
  // Vercel's Supabase integration can expose either NEXT_PUBLIC_* or SUPABASE_* names.
  ...
  if (!supabaseUrl || !supabaseUrl.startsWith("http")) {
    if (isProduction) {
      throw new Error(
        "Supabase URL is missing. Set NEXT_PUBLIC_SUPABASE_URL (or SUPABASE_URL) in Vercel environment variables.",
      );
    }
    console.warn(
      `[WARNING] NEXT_PUBLIC_SUPABASE_URL/SUPABASE_URL is missing or invalid (received: "${rawUrl}"). Falling back to local emulator.`,
    );
  ...
  if (!supabaseAnonKey || supabaseAnonKey.length < 10) {
    if (isProduction) {
      throw new Error(
        "Supabase anon key is missing. Set NEXT_PUBLIC_SUPABASE_ANON_KEY (or SUPABASE_ANON_KEY) in Vercel environment variables.",
      );
    }
    console.warn(
      `[WARNING] NEXT_PUBLIC_SUPABASE_ANON_KEY/SUPABASE_ANON_KEY is missing or too short. Falling back to dummy key.`,
    );
